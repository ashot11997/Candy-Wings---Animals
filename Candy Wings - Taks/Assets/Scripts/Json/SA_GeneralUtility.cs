...
namespace SA.Common.Util {
	...
	public static class General {
		...
		//	if (_dummy == null) {
		//		_dummy = new GameObject("DummyCoroutine").AddComponent<DummyObject>();
		//		UnityEngine.Object.DontDestroyOnLoad(_dummy.gameObject);
		//	}
		//	_dummy.StartCoroutine(routine);
		//}

		//private static DummyObject _dummy;
  ...
  //      public static Invoker Invoke(float time, Action callback, string name = "") {

		//	var i = Invoker.Create(name);
		//	i.StartInvoke(callback, time);

  //          return i;
		//}

		//public static CoroutineInvoker InvokeCoroutine (float time, Action callback, string name = "")
		//{
		//	var i = CoroutineInvoker.Create(name);
		//	i.StartInvoke(callback, time);
		//	return i;
		//}

  //      public static void ExcecuteOnNextFrame(Action action) {
  //          CoroutineQueue.Instance.ExcecuteOnNextFrame(action);
  //      }
        ...
        public static void StartTimer(string name) {
           // U.Log(string.Format("{0:HH:mm:ss tt}", DateTime.Now) + " Start timer: " + name, "user");
        ...
        public static float GetTime(string name) {
            ...
            if (s_timers.ContainsKey(name)) {
               // U.Log(string.Format("{0:HH:mm:ss tt}", DateTime.Now) + " Stop timer: " + name, "user");
		...
		public static void NoWarning(Exception ex) {
        ...
        public static T ParseEnum<T>(string value) {
			try {
                ...
                //U.LogWarning("Enum Parsing failed: " + ex.Message);
				return default(T);
			}
		}
